...
export const { authenticate, authorize, token } = koaOauthServer({
   model,
});
...
const router = (
   ...
   if (graphqlServer) {
      kRouter.all(
         ...
         async (context, next) => {
            ...
            log('user: ', user);
            const canRequest =
               user.permission === permissions.admnistrator ||
               user.permission === permissions.manager;
   ...
   kRouter.post(
      ...
      async (context, next) => {
         ...
         log(
            'context.state.oauth.token.user.username: ',
            context.state.oauth.token.user.username
         );
